...
export default class BKsearch extends Component {
    constructor(props) {
        super(props);
        this.state = {
          activePage: 2,
    ...
    handleClick(event) {
      ...
      componentDidMount(){
        ...
        .then(res => {
            console.log(this.state.set.toLowerCase())
            const a = res.data.filter(item => item.title.toLowerCase().search(this.state.set.toLowerCase())!=(-1));
            this.setState({number: a.length})
     ...
     const RenderTodos =()=>( 
      paginationdata.map((item, index) => {          ///mang  
          return (
            ...
            <Link to={{ pathname: `/${this.itemtolink(item.category)}/${item._id}`}}>
              ...
              <Grid>
                <div className="articles">
                  <header className="all-Event">
                    Tìm kiếm: {this.state.number} kết quả
                  </header>
                  <RenderTodos />
                  <div id="page-numbers">
                    <div>
        ...
        return (   
          ...
          <div>
        <Switch>
        <Route exact path={`${'/search'}/:Id`}    component={Intro}/>
