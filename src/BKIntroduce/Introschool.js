...
export default class BKIntroduce extends Component {
    ...
    handleClick(event) {
      ...
      componentDidMount(){
        ...
        .then(res => {
            const a = res.data.filter(item => item.category == "giới thiệu"&& item.sub_category == "Trường");
    ...
    const details = ({match}) =>(data.filter(data=>
     ...
     const RenderTodos =()=>( 
      paginationdata.map((item, index) => {          ///mang  
          return (
            <Link to={{ pathname: `//Gioithieu-truong/${item._id}`}}>
        ...
        return (   
          <div>
        <Switch>
          <Route exact path="/Gioithieu-truong" component={Intro} />
          <Route path="/Gioithieu-truong/:id" component={details} />
        </Switch>
      </div>
