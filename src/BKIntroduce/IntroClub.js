...
export default class BKIntroduce extends Component {
    ...
    handleClick(event) {
      ...
      componentDidMount(){
        ...
        .then(res => {
            const a = res.data.filter(item => item.category == "giới thiệu" && item.sub_category =="Câu lạc bộ đoàn hội");
    ...
    const details = ({match}) =>(data.filter(data=>
     ...
     const RenderTodos =()=>( 
      paginationdata.map((item, index) => {          ///mang  
          return (
            <Link to={{ pathname: `/Gioithieu-Cau-lac-bo/${item._id}`}}>
        ...
        return (   
          <div>
        <Switch>
          <Route exact path="/Gioithieu-Cau-lac-bo" component={Intro} />
          <Route path="/Gioithieu-Cau-lac-bo/:id" component={details} />
        </Switch>
      </div>
