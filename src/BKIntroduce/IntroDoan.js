...
export default class BKIntroduce extends Component {
    constructor(props) {
        super(props);
        this.state = {
          activePage: 2,
    ...
    handleClick(event) {
      ...
      componentDidMount(){
        ...
        .then(res => {
            const a = res.data.filter(item => item.category == "giới thiệu" && item.sub_category == "Đoàn thanh niên");
    ...
    const details = ({match}) =>(data.filter(data=>
     ...
     const RenderTodos =()=>( 
      paginationdata.map((item, index) => {          ///mang  
          return (
            <Link to={{ pathname: `/Gioithieu-doan-truong/${item._id}`}}>
        ...
        return (   
          <div>
        <Switch>
          <Route exact path="/Gioithieu-doan-truong" component={Intro} />
          <Route path="/Gioithieu-doan-truong/:id" component={details} />
        </Switch>
      </div>
