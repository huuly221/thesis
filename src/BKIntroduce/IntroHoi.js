...
export default class BKIntroduce extends Component {
    ...
    handleClick(event) {
      ...
      componentDidMount(){
        ...
        .then(res => {
            const a = res.data.filter(item => item.category == "giới thiệu" && item.sub_category == "Hội sinh viên");
    ...
    const details = ({match}) =>(data.filter(data=>
     ...
     const RenderTodos =()=>( 
      paginationdata.map((item, index) => {          ///mang  
          return (
            <Link to={{ pathname: `//Gioithieu-hoi-SV/${item._id}`}}>
        ...
        return (   
          <div>
        <Switch>
          <Route exact path="/Gioithieu-hoi-SV" component={Intro} />
          <Route path="/Gioithieu-hoi-SV/:id" component={details} />
        </Switch>
      </div>
