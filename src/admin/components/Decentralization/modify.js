 ...
 class ModifyAdmin extends Component {
    constructor(props) {
      ...
      onAfterDeleteRow(rowKeys) {
        axios.delete(`http://localhost:3001/api/admin/signup?_id=${rowKeys}`)
    ...
    createCustomButtonGroup = props => {
       ...
       onGrantAdmin() {
        ...
        axios.put(`http://localhost:3001/api/admin/signup?_id=${key}&is_admin=${true}`)
        .then(res => {
       ...
       onCancelAdmin() {
        var key = this.refs.table.state.selectedRowKeys[0];
        axios.put(`http://localhost:3001/api/admin/signup?_id=${key}&is_admin=${false}`)
        .then(res => {
          this.setState({ datapost: res.data});
      ...
      componentDidMount(){
        axios.get("http://localhost:3001/api/admin/signup")
        .then(res => {
          this.setState({ datapost: res.data});
          ...
          return (
           <div>
               <BootstrapTable
                        ...
                        <TableHeaderColumn dataField='is_admin' headerAlign='center' dataAlign='center'>Cộng tác viên</TableHeaderColumn>
                </BootstrapTable>
           </div>
          );
      }
