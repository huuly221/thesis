...
export default class News extends Component {
    ...
    handleClick(event) {
      ...
      componentDidMount(){
        ...
        .then(res => {
            const a = res.data.filter(item => item.category == "tin tức"&& item.sub_category =="Trường");
    ...
    const details = ({match}) =>(data.filter(data=>
     ...
     const RenderTodos =()=>( 
      paginationdata.map((item, index) => {          ///mang  
          return (
            <Link to={{ pathname: `/Tintuc-truong/${item._id}`}}>
        ...
        return (   
          <div>
        <Switch>
          <Route exact path="/Tintuc-truong" component={New} />
          <Route path="/Tintuc-truong/:id" component={details} />
        </Switch>
      </div>
