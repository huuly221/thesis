...
export default class News extends Component {
    ...
    handleClick(event) {
      ...
      componentDidMount(){
        ...
        .then(res => {
            const a = res.data.filter(item => item.category == "tin tức" && item.sub_category =="hoạt động phong trào");
    ...
    const details = ({match}) =>(data.filter(data=>
     ...
     const RenderTodos =()=>( 
      paginationdata.map((item, index) => {          ///mang  
          return (
            <Link to={{ pathname: `/Tintuc-phong-trao/${item._id}`}}>
        ...
        return (   
          <div>
        <Switch>
          <Route exact path="/Tintuc-phong-trao" component={New} />
          <Route path="/Tintuc-phong-trao/:id" component={details} />
        </Switch>
      </div>
