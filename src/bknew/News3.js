...
export default class News extends Component {
    ...
    handleClick(event) {
      ...
      componentDidMount(){
        ...
        .then(res => {
            const a = res.data.filter(item => item.category == "tin tức" && item.sub_category =="hoạt động quyên góp");
    ...
    const details = ({match}) =>(data.filter(data=>
     ...
     const RenderTodos =()=>( 
      paginationdata.map((item, index) => {          ///mang  
          return (
            <Link to={{ pathname: `/Tintuc-quyen-gop/${item._id}`}}>
        ...
        return (   
          <div>
        <Switch>
          <Route exact path="/Tintuc-quyen-gop" component={New} />
          <Route path="/Tintuc-quyen-gop/:id" component={details} />
        </Switch>
      </div>
