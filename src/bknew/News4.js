...
export default class News extends Component {
    ...
    handleClick(event) {
      ...
      componentDidMount(){
        ...
        .then(res => {
            const a = res.data.filter(item => item.category == "tin tức" && item.sub_category =="công tác tổ chức");
    ...
    const details = ({match}) =>(data.filter(data=>
     ...
     const RenderTodos =()=>( 
      paginationdata.map((item, index) => {          ///mang  
          return (
            <Link to={{ pathname: `/Tintuc-cong-tac-to-chuc/${item._id}`}}>
        ...
        return (   
          <div>
        <Switch>
          <Route exact path="/Tintuc-cong-tac-to-chuc" component={New} />
          <Route path="/Tintuc-cong-tac-to-chuc/:id" component={details} />
        </Switch>
      </div>
