...
import {  Link} from "react-router-dom";
 class ModifyAdmin extends Component {
    constructor(props) {
       ...
       this.handleInsertedRow = this.handleInsertedRow.bind(this);
        ...
        this.state = {
         data:[],
         datapost: [],
         set: this.props.Id, ////code event pass from manager.js(select event and pass code event)
         number:0
      ...
      onAfterDeleteRow(rowKeys) {
        axios.delete(`http://localhost:3001/api/eventsuser?_id=${rowKeys}`)
        .then(
      ...
      handleInsertedRow(row){
        ...
        axios.post("http://localhost:3001/api/eventsuser",this.state.thisevent)
        .then(
    ...
    createCustomButtonGroup = props => {
      ...
      componentDidMount(){
        axios.get(`http://localhost:3001/api/eventsuser?code=${this.state.set}`)
        .then(res => {
      ...
      render() {
        ...
        const selectRowProp = {
          ...
          return (
            ...
            <h3>Số người đăng kí: {this.state.datapost.length} người</h3>
            ...
            <div>
               <BootstrapTable
                        ...
                        <TableHeaderColumn dataField='mssv'  filter={ { type: 'TextFilter', delay: 0 } } headerAlign='center' dataAlign='center'>MSSV</TableHeaderColumn>
