...
class BKEvent extends Component {
  ...
  handleClick(event) {
    ...
    componentDidMount(){
      ...
      .then(res => {
          const a = res.data.filter(item => item.category == "sự kiện" && item.sub_category ==  "cơ sở");
  ...
  const details = ({match}) =>(data.filter(data=>
   ...
   const RenderTodos =()=>( 
    paginationdata.map((item, index) => {          ///mang  
        return (
          <Link to={{ pathname: `/Sukien-co-so/${item._id}`}}>
      ...
      <Switch>
        <Route exact path="/Sukien-co-so" component={Event} />
        <Route path="/Sukien-co-so/:id" component={details} />
