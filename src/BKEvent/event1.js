...
class BKEvent extends Component {
  ...
  handleClick(event) {
    ...
    componentDidMount(){
      ...
      .then(res => {
          const a = res.data.filter(item => item.category == "sự kiện" &&item.sub_category == "ĐTN - HSV trường");
  ...
  const details = ({match}) =>(data.filter(data=>
   ...
   const RenderTodos =()=>( 
    paginationdata.map((item, index) => {          ///mang  
        return (
          <Link to={{ pathname: `/Sukien-DTN-HSV/${item._id}`}}>
      ...
      <Switch>
        <Route exact path="/Sukien-DTN-HSV" component={Event} />
        <Route path="/Sukien-DTN-HSV/:id" component={details} />
