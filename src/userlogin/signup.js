    ...
    handleSubmit(e) {
        ...
        if (!name || !pass||!tel) {
          ...
          if(sessionStorage.getItem("stack-mssv")){
            var nameVn = sessionStorage.getItem("stack-name")
            var mssv =sessionStorage.getItem("stack-mssv")
            var email = mssv +"@hcmut.edu.vn";   /// get eamil from logingoogle to signup
            this.handleUserSubmit({email: email , mssv:mssv ,display_name: name, password: pass, phone: tel,full_name:nameVn });
      ...
      handleUserSubmit(user) {
       ...
       ////chuyen hoa du lieu
        ...
        axios.post(this.props.url, user)
          ...
          sessionStorage.setItem("display_name",this.state.name);     
          dispatch({type: 'LOG_IN', display_name:this.state.name});  
      }
